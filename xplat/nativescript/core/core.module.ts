...
import { CoreModule, PlatformLanguageToken, PlatformWindowToken } from '@dataform/core';
import { throwIfAlreadyLoaded } from '@dataform/utils';
...
export class DataformCoreModule {
  constructor(
    @Optional()
    @SkipSelf()
    parentModule: DataformCoreModule
  ) {
    throwIfAlreadyLoaded(parentModule, 'DataformCoreModule');
  }
}
